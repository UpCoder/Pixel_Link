...
from nets.lovasz_losses_tf import lovasz_softmax
...
class PixelLinkNet_multiphase_multislice_clstm_mask(object):
    def __init__(self, nc_inputs, art_inputs, pv_inputs, mask_input, is_training, batch_size_ph):
    ...
    def _fuse_feat_layers(self):
        ...
        if config.feat_fuse_type == FUSE_TYPE_cascade_conv1x1_upsample_sum:
            self.pixel_cls_logits = self._fuse_by_cascade_conv1x1_upsample_sum(
                2, scope='pixel_cls')
            self.pixel_seg_logits = self._fuse_by_cascade_conv1x1_upsample_sum(
                config.num_tumor_type, scope='pixel_seg'
            )
            ...
            self.pixel_cls_logits = self._score_layer(base_map,
                                                      2, scope='pixel_cls')
            self.pixel_seg_logits = self._score_layer(base_map,
                                                      config.num_tumor_type, scope='pixel_seg')
    ...
    def _logits_to_scores(self):
        self.pixel_seg_score = tf.nn.softmax(self.pixel_seg_logits)
    ...
    def build_loss(self, pixel_cls_labels, pixel_cls_weights,
        ...
        with tf.name_scope('pixel_link_loss'):
            ...
            _, h, w, channel = self.pixel_seg_score.get_shape().as_list()
            # print('the pixel_seg_score is ', self.pixel_seg_score)
            # print('the mask input is ', tf.squeeze(self.mask_input, axis=3))
            # print('the mask input is ', tf.squeeze(tf.image.resize_images(self.mask_input, [h, w],
            #                                                      tf.image.ResizeMethod.NEAREST_NEIGHBOR), axis=3))
            # self.seg_loss = lovasz_softmax(self.pixel_seg_score,
            #                                tf.squeeze(tf.image.resize_images(self.mask_input, [h, w],
            #                                                                  tf.image.ResizeMethod.NEAREST_NEIGHBOR),
            #                                           axis=3),
            #                                per_image=True)
            from losses import compute_seg_loss
            original_seg_logists = tf.image.resize_images(self.pixel_seg_logits, [512, 512])
            print('mask input is ', self.mask_input)
            print('the original seg logists is ', original_seg_logists)
            # seg_loss, dice_tensor, dice_tensors = compute_seg_loss(tf.squeeze(self.mask_input), original_seg_logists, num_classes=5)
            seg_loss = compute_seg_loss(tf.squeeze(self.mask_input), original_seg_logists,
                                                                   num_classes=5)
            tf.add_to_collection(tf.GraphKeys.LOSSES, seg_loss)
        ...
        if do_summary:
            ...
            # tf.summary.scalar('average_dice', dice_tensor)
