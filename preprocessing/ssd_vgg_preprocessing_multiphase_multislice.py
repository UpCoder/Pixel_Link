...
import cv2
import util
from preprocessing import tf_image_multiphase_multislice
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train', [nc_image, art_image, pv_image, labels, bboxes]):
        ...
        if USE_ROTATION:
            ...
            def rotate():
                ...
                return tf_image_multiphase_multislice.random_rotate90_multiphase_multislice(nc_image, art_image,
                                                                                            pv_image, bboxes, xs, ys)
        ...
        if MAX_EXPAND_SCALE > 1:
            ...
            def expand():
                ...
                return tf_image_multiphase_multislice.resize_image_bboxes_with_crop_or_pad_multiphase_multislice(
        ...
        nc_dst_image = tf_image_multiphase_multislice.resize_image(nc_dst_image, out_shape,
                                          method=tf.image.ResizeMethod.BILINEAR,
                                          align_corners=False)
        tf_summary_image(nc_dst_image, bboxes, 'image_shape_distorted/nc')

        art_dst_image = tf_image_multiphase_multislice.resize_image(art_dst_image, out_shape,
                                              method=tf.image.ResizeMethod.BILINEAR,
                                              align_corners=False)
        tf_summary_image(art_dst_image, bboxes, 'image_shape_distorted/art')

        pv_dst_image = tf_image_multiphase_multislice.resize_image(pv_dst_image, out_shape,
                                             method=tf.image.ResizeMethod.BILINEAR,
                                             align_corners=False)
    ...
    with tf.name_scope(scope):
        ...
        if do_resize:
            if resize == Resize.NONE:
                ...
                nc_image = tf_image_multiphase_multislice.resize_image(nc_image, out_shape,
                                                 method=tf.image.ResizeMethod.BILINEAR,
                                                 align_corners=False)
                art_image = tf_image_multiphase_multislice.resize_image(art_image, out_shape,
                                                  method=tf.image.ResizeMethod.BILINEAR,
                                                  align_corners=False)
                pv_image = tf_image_multiphase_multislice.resize_image(pv_image, out_shape,
                                                 method=tf.image.ResizeMethod.BILINEAR,
                                                 align_corners=False)
